...
const Contact = () => {
  ...
  return (
    <div id="contact" className={`z-0 ${isLight ? "bg-hero-pattern-light" : "bg-hero-pattern-dark"} bg-cover bg-no-repeat bg-center`}>
      <div className={`absolute inset-0 bg-gradient-to-b from-transparent from-60% ${isLight ? "to-white" : "to-black"}`}></div>
      <section className={`relative shrink-0 w-full h-screen mx-auto ${styles.paddingX} pt-[120px] max-w-7xl mx-auto flex items-start justify-center`}>
        <div className="relative w-full h-auto flex flex-col lg:flex-row items-center justify-between mt-10">
          <div className="lg:w-1/2 w-full p-8">
          <h2 className={`text-3xl font-bold mb-4 ${isLight ? "text-black-100" : "text-white-100"}`}>Contact</h2>
            <h3 className={`text-xl font-bold mb-6 ${isLight ? "text-black-100" : "text-white-100"}`}>Write <span className="text-green-500">anytime</span></h3>
            <form
              className="bg-white p-8 rounded-lg shadow-md"
              noValidate
              action="https://formspree.io/f/xrgnnrzl"
              method="POST"
            >
              ...
              <label className="block mb-4">
                ...
                <textarea
                  className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm ${errors.message ? 'border-red-500' : ''}`}
                  onChange={handleInput}
              ...
              <button
                ...
                className={`mt-4 px-4 py-2 rounded-md bg-green-500 text-white ${!isFormValid ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-700'}`}
                type="submit"
                value="send"
              >
          ...
          <div className="lg:w-1/2 w-full p-8 flex items-center justify-center">
            <img src={contact} alt="Contact" className="w-full h-auto object-contain" />
          </div>
        </div>
      </section>
    </div>
